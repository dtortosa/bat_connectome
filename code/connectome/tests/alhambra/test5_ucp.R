#test if number of the suggested BAT genes by literature included in BAT connectome is higher than expected by chance. 
...
BAT_described_indirect = all_genes[which(all_genes %in% func[which(func$FUN==2),]$GENES)]
...
for(i in 1:n_iter){
    ...
    random_genes = all_genes[sample(1:length(all_genes), length(BAT_described_indirect))]
    ...
    if(length(random_in_BAT_connectome) == length(BAT_described_indirect)){
...
### save results
write.table(results, "/SCRATCH/UGR002/dsalazar/ucp1_connectome/results/results_test_5.txt", col.names = FALSE, row.names = FALSE, sep="\t")
