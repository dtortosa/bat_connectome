...
#Test that the Biological distance between known BAT genes inside the connectome are smaller than that of known BAT and random genes.

#we take two BAT genes, we take the median distance between known within the connectome and compare them with the distance between known genes and random sets of genes
...
### remove UCP1 from known BAT genes. We only want the 60 known BAT genes, not the core of the connectome. 
known_bat_genes = known_bat_genes[which(known_bat_genes != "UCP1")]
#check
!"UCP1" %in% known_bat_genes
...
#select the distances between known BAT genes
known_BAT_distance = all_pairs[which(all_pairs$gen1 %in% known_bat_genes & all_pairs$gen2 %in% known_bat_genes),]
#number of known BAT distances
nrow(known_BAT_distance)
#check
summary(known_BAT_distance$gen1 %in% known_bat_genes)
summary(known_bat_genes %in% known_BAT_distance$gen1)
summary(known_BAT_distance$gen2 %in% known_bat_genes)
summary(known_bat_genes %in% known_BAT_distance$gen2)
...
#median(all_pairs[which(all_pairs$gen2 %in% known_bat_genes & all_pairs$gen1 %in% unknown_bat_genes),]$biological_distance) == median(known_unknown_BAT_distance$biological_distance) #exactly the same distance
    #this makes sense for the test6 not here.
...
#IMPORTANT. I did a general check for the test 6 (uses the same approach with different sets), inverting the order of gen1 and gen2 and I get the same results in version 2 of the script.

#subset all pairs to reduce the size.
all_pairs_subset = all_pairs[which(all_pairs$gen1 %in% known_bat_genes & !all_pairs$gen2 %in% known_bat_genes) ,which(colnames(all_pairs) %in% c("gen1", "gen2", "biological_distance"))] 
    #we only want the rows for which the gen1 is a known-bat gene and the gene2 is NOT an known bat gene, becuase we want only the random. In this case (different to test 6), we are not testing candidate genes, so we can included as controls (this in any reduce would reduce power...). This is 60 known-BAT respect the rest. Therefore, we do not need "& !all_pairs$gen2 %in% known_bat_genes"
    #We test known-BAT with random genes, so we need biological distance columns. We only have now random genes in the second column and known-bat genes in the first one, we can just take random rows.
str(all_pairs_subset)

#check that known is in gene 1, but not in gene 2
summary(known_bat_genes %in% all_pairs_subset$gen1)
summary(all_pairs_subset$gen1 %in% known_bat_genes)
summary(!known_bat_genes %in% all_pairs_subset$gen2)
summary(!all_pairs_subset$gen2 %in% known_bat_genes)


#remove the gene columns
all_pairs_subset = all_pairs_subset[, -which(colnames(all_pairs) %in% c("gen1", "gen2"))]
str(all_pairs_subset)
...
for(i in 1:1000000){
    ...
    random_distance_rows = sample(1:length(all_pairs_subset), nrow(known_BAT_distance), replace=FALSE) 
    ...
    test7 = append(test7, median(random_known_BAT_distance) <= median(known_BAT_distance$biological_distance))
}
...
pval = prop.test(x=length(which(test7 == TRUE)), n=length(test7)) #https://stats.stackexchange.com/questions/167164/test-of-equal-proportions-with-zero-successes
pval

#save results
write.table(test7, "/media/dftortosa/Windows/Users/dftor/Documents/diego_docs/science/open_projects/connectome/results/results_2020/tests/test_7/results_test_7_v3.txt", col.names = FALSE, row.names = FALSE, sep="\t")

#save the results
save.image("/media/dftortosa/Windows/Users/dftor/Documents/diego_docs/science/open_projects/connectome/results/results_2020/rdata/test7_ucp_v1.RData")
require(plyr)