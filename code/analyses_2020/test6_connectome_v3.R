...
#Respect to version 1: I have added a new condition to select random distances. We selected distance between known-BAT genes (gen1) and genes not included in the connectome (gen1=random, known or candidate genes). In the last version, we could have known-BAT genes in gen2. This is not very problematic becuase if you include BAT genes, you get lower distance but no random set was closer than known-candidate. This is just in case, and to be precise with what we explain in the paper.
...
#number of known - unknown BAT distances
nrow(known_unknown_BAT_distance)
#check
summary(known_unknown_BAT_distance$gen1 %in% known_bat_genes)
summary(known_bat_genes %in% known_unknown_BAT_distance$gen1)
summary(!known_unknown_BAT_distance$gen1 %in% unknown_bat_genes)
summary(!unknown_bat_genes %in% known_unknown_BAT_distance$gen1)
summary(known_unknown_BAT_distance$gen2 %in% unknown_bat_genes)
summary(unknown_bat_genes %in% known_unknown_BAT_distance$gen2)
summary(!known_unknown_BAT_distance$gen2 %in% known_bat_genes)
summary(!known_bat_genes %in% known_unknown_BAT_distance$gen2)
...
all_pairs_subset = all_pairs[which(all_pairs$gen1 %in% known_bat_genes & !all_pairs$gen2 %in% unknown_bat_genes & !all_pairs$gen2 %in% known_bat_genes) ,which(colnames(all_pairs) %in% c("gen1", "gen2", "biological_distance"))] 
    #we only want the rows for which the gen1 is a known-bat gene and the gene2 is NOT an known or candidate bat gene (NO connectome), becuase we want only the random. We test known-BAT with random genes, so we need biological distance columns. We only have now random genes in the second column and known-bat genes in the first one, we can just take random rows.
    #IMPORTANT NOTE. In gen2 we can have known-BAT genes, but this is not problematic. If we have in a random set of distances, distances between known-BAT genes, this would decrease the median, decreasing the power. Even though we not have ANY case where the median random distance was higher than the known-candidate distance (see below). In addition, we are using the median which is less sensitive to outliers, like can be a few known-known distances within a subset of 65000 random distances. If you want to change it you have to add a new conditions "& !all_pairs$gen2 %in% known_bat_genes"
        #check that two conditions works
        #check = c(1,2,3,4,5,6)
        #check[which(!check %in% c(1,2) & !check %in% c(5,6))]
str(all_pairs_subset)

#check that NO of the connectome genes are included in the second columns of genes of the distances between known BAT (gen1) and random (gen2)
summary(!unknown_bat_genes %in% all_pairs_subset$gen2)
summary(!all_pairs_subset$gen2 %in% unknown_bat_genes)
summary(!known_bat_genes %in% all_pairs_subset$gen2)
summary(!all_pairs_subset$gen2 %in% known_bat_genes)
#check that known is in gen1
...
#save results
write.table(test6, "/media/dftortosa/Windows/Users/dftor/Documents/diego_docs/science/open_projects/connectome/results/results_2020/tests/test_6/results_test_6_v3.txt", col.names = FALSE, row.names = FALSE, sep="\t")

#save the results
save.image("/media/dftortosa/Windows/Users/dftor/Documents/diego_docs/science/open_projects/connectome/results/results_2020/rdata/test6_ucp_v3.RData")
require(plyr)