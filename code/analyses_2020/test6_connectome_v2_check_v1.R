##############################################################################################
####################################### TEST 6 CHECK GEN1 - GENE 2 ########################### 
##############################################################################################

#THIS IS EXACTLY like test6_connectome_v2_check.R but inverting the order of gen1 and gen2 when selecting distances between known BAT genes and random genes. In that way, I wanted to check that the order is not important, the distance when selecting gen1=UCP1 and gen2=UCP2 is the same than gen2=UCP1 and gen1=UCP2.

#Instructions from test6_connectome_v2_check.R
	...
	#in the previous version I made gene to gene comparisons, but it is too slow, in addition, it does not make sense to make a sampling of the genes within the connectome, because we have a very low number. You should compare each one with random sets, or the median.
...
#subset all pairs to reduce the size, but THIS TIME GEN2 ARE known-BAT genes and gen1 are random gene
all_pairs_subset = all_pairs[which(all_pairs$gen2 %in% known_bat_genes & !all_pairs$gen1 %in% unknown_bat_genes) ,which(colnames(all_pairs) %in% c("biological_distance"))] #we only want the rows for which the gen2 is a known-bat gene and the gene1 is NOT an unknown bat gene, becuase we want only the random. We test known-BAT with random genes, so we need biological distance columns. We only have now random genes in the second column and known-bat genes in the first one, we can just take random rows.
...
#save the results
save.image("/media/dftortosa/Windows/Users/dftor/Documents/diego_docs/science/open_projects/connectome/results/results_2020/rdata/test6_ucp_v2_check.RData")
require(plyr)